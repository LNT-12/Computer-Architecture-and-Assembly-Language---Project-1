...
string HexToBinString(string hex) {
	static map<char, string> HexCharToBin;
	...
	for (int i = 0; i < hex.length(); ++i)
		for (int j = 0; j < 4; ++j)
			res += HexCharToBin[hex[i]][j];

	return res;
}

QInt BinStringToDec(string binString) {
	while (binString.length() < 128)
